...
export default function UserMetaCard() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (result.success) {
      showToast("success", "Perfil actualizado", "Tus cambios se guardaron correctamente");
      closeModal();
    } else {
      showToast("error", "Error al actualizar", result.error || "No se pudo guardar el perfil");
    }
  };
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file.type.startsWith("image/")) {
      showToast("warning", "Archivo inválido", "Por favor selecciona una imagen");
      return;
    }

    // Validate file size (max 5MB)
    if (file.size > 5 * 1024 * 1024) {
      showToast("warning", "Archivo muy grande", "La imagen debe ser menor a 5MB");
      return;
    }
    ...
    if (result.success) {
      showToast("success", "Foto actualizada", "Tu foto de perfil se actualizó correctamente");
    } else {
      showToast("error", "Error al subir imagen", result.error || "No se pudo subir la imagen");
    }
  };
                ...
                <div className="grid grid-cols-1 gap-x-6 gap-y-5 lg:grid-cols-2">
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      defaultValue={formData.twitter_url}
