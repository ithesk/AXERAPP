-- =====================================================
-- UPDATE: Notification triggers use robust org member lookup
-- Description: Agrega helper para resolver usuarios por email o nombre
--              dentro de una organización y actualiza los triggers de
--              notificaciones para usarlo.
-- Date: 2025-10-18
-- =====================================================

BEGIN;

-- Helper para resolver ID de usuario dentro de una organización
...
BEGIN
  ...
  WITH member_candidates AS (
    SELECT
      ...
      LOWER(TRIM(CONCAT_WS(' ', COALESCE(p.first_name, ''), COALESCE(p.last_name, '')))) AS full_name_lower
  ...
  WHERE email_lower = LOWER(v_clean_identifier)
     OR (full_name_lower IS NOT NULL AND full_name_lower <> '' AND full_name_lower = LOWER(v_clean_identifier))
  LIMIT 1;
...
BEGIN
  IF OLD.tecnico_asignado IS DISTINCT FROM NEW.tecnico_asignado THEN
    ...
    IF v_new_technician_id IS NOT NULL AND v_new_technician_id != NEW.usuario_id THEN
      PERFORM public.create_notification(
        p_org_id := NEW.org_id,
...
DO $$
BEGIN
  RAISE NOTICE '✅ Notification triggers updated to use find_org_member_user_id helper';
END $$;
